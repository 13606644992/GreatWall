...
- (void)setupPageView{
    ...
    //3,初始化页码显示器
    //3.1设置页码显示器参数
    AEPageModel *model = [AEPageModel pageModelWithPageImage:[self pageImage] currentImage:[self currentImage] pages:self.downloadImages.count gapsize:4*WEIGHT imageSize:CGSizeMake(5*WEIGHT, 5*WEIGHT)];
...
//    UIGraphicsEndImageContext();
    return [UIImage imageNamed:@"bannerlunbo"];
}
...
//     CGContextAddEllipseInRect(ctx, CGRectMake(80, 80, 160, 100));
    //取图
...
- (void)layoutSubviews{
    ...
    if(self.pageLocation == AEPageViewLocationCenter){
        self.pageView.center = CGPointMake(self.frame.size.width * 0.5, self.frame.size.height * 0.9);
