...
#pragma mark - 数据请求
- (void)getData{
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:@{@"userId":[[NSUserDefaults standardUserDefaults]objectForKey:@"GJ_userId"],@"sts":@"0",@"couponType":@"0"}];
    [GJAFNetWork POST:URL_ALIANG params:dic method:@"getUserCouponsCount" tpye:@"post" success:^(NSURLSessionDataTask *task, id responseObject) {
        if ([responseObject[@"respCode"]isEqualToString:@"000000"]) {
            NSLog(@"红包数量=====%@",responseObject);
            NSDictionary *output = responseObject[@"output"];
            NSNumber *yiShiYong = output[@"couponUsedValue"];
            NSNumber *weiShiYong = output[@"couponNoUsedValue"];
            NSNumber *yiGuoQi = output[@"couponExpiredValue"];
            NSLog(@"%@,%@,%@",yiGuoQi,yiShiYong,weiShiYong);
            [_weishiyongBtn setTitle:[NSString stringWithFormat:@"未使用(%@)", weiShiYong] forState:UIControlStateNormal];
            [_shiyongjiluBtn setTitle:[NSString stringWithFormat:@"使用记录(%@)", weiShiYong] forState:UIControlStateNormal];
            [_yiguoqiBtn setTitle:[NSString stringWithFormat:@"已过期(%@)", yiGuoQi] forState:UIControlStateNormal];
        }
