...
@interface YiChuDanGeXianVC ()
@property (nonatomic, strong) NSMutableArray *dataArray;
@property (nonatomic, assign) NSInteger page;
@property (nonatomic, assign) BOOL isTop;
@end

@implementation YiChuDanGeXianVC
{
...
#pragma mark - 数据请求
- (void)getData{
    NSMutableDictionary *paramers = [NSMutableDictionary dictionaryWithDictionary:@{@"userId" : [[NSUserDefaults standardUserDefaults] objectForKey:@"GJ_userId"], @"policySts" : @"20",@"classType":@"2", @"pageSize" : @"10", @"pageIndex" : [NSString stringWithFormat:@"%ld", (long)self.page]}];
    NSLog(@"%@", paramers);
    [GJAFNetWork POST:URL_ALIANG params:paramers method:@"getPolicies" tpye:@"post" success:^(NSURLSessionDataTask *task, id responseObject) {
        NSLog(@"个保--已出单--responseObject==%@", responseObject);
