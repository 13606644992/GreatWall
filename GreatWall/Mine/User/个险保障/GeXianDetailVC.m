...
@property (nonatomic, strong)NSMutableArray *insureAmountArray;
@end
...
- (NSMutableArray *)insureAmountArray{
    if (!_insureAmountArray) {
        _insureAmountArray = [[NSMutableArray alloc]init];
    }
    return _insureAmountArray;
}
...
- (void)getData{
    ...
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:@{@"userId":userId,@"detailId":self.detailID,@"classType":@"2"}];
    [GJAFNetWork POST:URL_ALIANG params:dic method:@"getTradeDetail" tpye:@"post" success:^(NSURLSessionDataTask *task, id responseObject) {
        NSLog(@"个保详情responseObject===========%@",responseObject);
        if ([responseObject[@"respCode"]isEqualToString:@"000000"]) {
            ...
            [self.detailModel.typeList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [fenleiModel.insureList enumerateObjectsUsingBlock:^(id  _Nonnull obj2, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    NSString *insureAmount = obj2[@"insureAmount"];
                    [self.insureAmountArray addObject:insureAmount];
...
//    NSLog(@"+++++++++++%@",beiBaoXianRen.idType);
    BaoZhangNeiRongFenLeiModel *model = [[BaoZhangNeiRongFenLeiModel alloc] init];
    BaoZhangNeiRongModel *mod = [[BaoZhangNeiRongModel alloc] init];
    if (self.detailModel.typeList.count > 0) {
       model = self.detailModel.typeList[0];
    }
    if (indexPath.section == 0) {
        mod = model.insureList[indexPath.row];
        if (indexPath.row == 0) {
        ...
        cell.contentLab.text = [NSString stringWithFormat:@"%.2f",[mod.insureAmount floatValue]];
    }else if (indexPath.section == 1){
        if (indexPath.row == 0){
            cell.titleLab.text = @"支付保费";
            cell.contentLab.text = [NSString stringWithFormat:@"%.2f",[self.detailModel.tradePremium floatValue]];
        }else if (indexPath.row == 1){
            cell.titleLab.text = @"保障开始时间";
            cell.contentLab.text = self.detailModel.startTime;
        }else if (indexPath.row == 2){
            cell.titleLab.text = @"保障结束时间";
            cell.contentLab.text = self.detailModel.endTime;
        }
        ...
        if (indexPath.row == 0){
            ...
            NSLog(@"idType===============%@",beiBaoXianRen.idType);
            ...
            cell.contentLab.text = beiBaoXianRen.birthday;
...
//        BaoZhangNeiRongFenLeiModel *model = self.detailModel.typeList[0];
        cell.contentLab.text = model.typeName;
