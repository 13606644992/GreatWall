...
@property (nonatomic, assign) NSUInteger lastIndex;                     //上次个数
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    self.lineLab.frame = CGRectMake(0, titleScrollViewToTop+_tabBarHeight-0.5, kScreenWidth, 0.5);
...
- (BOOL)_isTitlesContentNotLessThanViewWidth:(NSArray *)dataArray {
    ...
    for (int index = 0; index < dataArray.count; index ++) {
        ...
        CGFloat calculate = [NSObject widthFromString:model.controllerTitle withFont:[UIFont systemFontOfSize:14*([[UIScreen mainScreen] bounds].size.width/375)] constraintToHeight:self.tabBarHeight] +16.0f;
...
- (void)_initMethod {
    ...
    //color
    self.selectedTitleColor = [LYColor colorWithHexString:@"#2dce8f"];
    self.unselectedTitleColor = [LYColor colorWithHexString:@"#484c4a"];
...
- (void)_updateSelectedTitle:(NSUInteger)selectedIndex {
    for (UILabel *label in self.titleScrollView.subviews) {
        ...
        label.font = (selectedIndex == label.tag)?[UIFont systemFontOfSize:14*([[UIScreen mainScreen] bounds].size.width/375)*self.fontAmplification]:[UIFont systemFontOfSize:14*([[UIScreen mainScreen] bounds].size.width/375)];
...
-(UILabel *)lineLab
{
    if (!_lineLab) {
        ...
        _lineLab.backgroundColor = [LYColor colorWithHexString:@"#e3e3e3"];
    }
    return _lineLab;
}
- (UIView *)maskView {
